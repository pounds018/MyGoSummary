...
1. 数组是同一种数据类型的集合, 一旦声明了 `长度固定,元素类型固定`
2. 数组中如果不初始化,其元素使用的是该数据类型的默认值
3. 使用的时候注意不要越界了
4. go中数组是值类型,要修改数组的值需要传递引用类型
5. `[n]*T`表示指针数组，`*[n]T`表示数组指针 。
6. [1]int和[3]int 并不是同一种类型,其他数据类型的数组也是一样的.与Java不太相同，在go语言中，相同类型但是数组容量不同，也被视为不同的数组类型，只有两个条件都相同才是同一类型。

##### 2.切片: []数据类型

**切片**: 拥有相同类型元素的可变长度的序列.它是基于数组做的一层封装,`支持自动扩容`,是一个引用类型.
